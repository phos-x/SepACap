...
* **`create_scp/`**: SCP (Script) files are essentially lightweight text files that act as pointers. Instead of loading thousands of heavy audio files into the computer's RAM at once, these scripts generate lists of file paths. The dataloader reads the SCP file and fetches the exact audio chunk it needs at that exact millisecond, saving massive amounts of memory.
* **`figure/`**: A storage directory for generated graphs, plots, and visual representations of your dataset (like spectrogram visualizations or data distribution charts).