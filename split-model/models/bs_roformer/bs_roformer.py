...
class BSRoformer(Module):
    ...
    def __init__(
        ...
        # --- EXTENSION: Placeholder for Context Injector ---
        # This will be populated dynamically by SATBSeparatorWrapper if enabled in config.
        # Initializing as None ensures baseline compatibility.
    ...
    def forward(
            ...
            context_embeddings=None  # --- EXTENSION: Accept context embeddings (SSL/LLM) ---
    ):
        """
        einops
        ...
        # --- EXTENSION: CONTEXT INJECTION (SSL / LLM) ---
        # Inject the context dynamically aligned via cross-attention right before the heavy transformer blocks.
        if context_embeddings is not None and getattr(self, 'context_injector', None) is not None:
            # x shape after band_split: (b, t, bands, d)
            # Average across bands to get a global frame representation for the cross-attention queries
            x_mean = x.mean(dim=2)  # shape: (b, t, d)
            
            if self.use_torch_checkpoint:
                injected_context = checkpoint(self.context_injector, x_mean, context_embeddings, use_reentrant=False)
            else:
                injected_context = self.context_injector(x_mean, context_embeddings)
            
            # Broadcast the cross-attention output back across the bands via residual connection
            x = x + injected_context.unsqueeze(2)
        # ------------------------------------------------
